...
// A simple custom media query hook
...
// Optional sort mapping for demonstration
const sortMapping = {
    "Самые подходящие": 1,
...
export default function Home() {
    ...
    useEffect(() => {
        const fetchLocalData = async () => {
            try {
                ...
                setAnnouncements(data) // Update state with local JSON
            } catch (error: any) {
    ...
    const renderSkeletonCard = () => (
        <div className="relative">
            ...
            >
                <Skeleton variant="rectangular" width={240} height={130} style={{ borderRadius: "10px" }} />
    ...
    // Example toggle back to list view
            ...
            {isMapDropdownOpen ? (
                ...
                <div className="flex-grow w-screen xl:px-10 2xl:px-10 md:px-10">
                    ...
                    <div className="flex flex-col w-full md:flex-row justify-around gap-[20px] md:gap-[25px]" >
                        <div className={isMobile ? "w-full flex flex-row h-[60px]" : "md:min-w-[300px] lg:min-w-[350px] xl:min-w-[450px]"} style={{boxShadow: "0px 4px 10px 0px rgba(0, 0, 0, 0.2)"}}>
                            {isMobile && (
                                <button
                                    type="button"
                                        ...
                                        {isSortDropdownOpen && (
                                            <div className="absolute top-[30px] left-[-10px] bg-white space-y-[12px] min-w-[200px] rounded-[5px] text-left z-10">
                                                <ul className="flex flex-col">
                                                    {["Самые подходящие", "По возрастанию цены", "По новизне", "По убыванию цены"].map(
                                                        (sortOption) => (
                                                            <li
                                                                key={sortOption}
                                                                onClick={() => {
